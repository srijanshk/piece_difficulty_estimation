...
def compute_optimized_score_features(score: Union[pt.score.Score, pt.score.Part, np.ndarray]) -> np.ndarray:
    """Compute a more focused set of features."""
    if isinstance(score, (pt.score.Score, pt.score.Part)):
        na = score.note_array()
    elif isinstance(score, np.ndarray):
        na = score
    ...
    vertical = compute_vertical_neighbors(na)
    
    # Simplified rhythm features
    durations = na['duration_beat']
    unique_durations = len(np.unique(durations))
    grace_notes = np.mean(na['is_grace']) if 'is_grace' in na.dtype.names else 0
    ...
    # Hand coordination (focused on extreme spans)
    spans = []
    unique_onsets = np.unique(na['onset_beat'])
    for onset in unique_onsets:
        simultaneous = na[na['onset_beat'] == onset]
        if len(simultaneous) > 1:
            span = np.max(simultaneous['pitch']) - np.min(simultaneous['pitch'])
            spans.append(span)
    max_span = np.max(spans) if spans else 0
    
    # Key signature complexity
    key_complexity = np.abs(na['ks_fifths'][0]) if 'ks_fifths' in na.dtype.names else 0
    ...
    features = np.array([
        ...
        max_span,
        key_complexity,
        *vertical  # Unpacking vertical neighbor features
    ])
